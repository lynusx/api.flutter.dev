@docImport 'action_buttons.dart'; @docImport 'animated_icons.dart'; @docImport 'icon_button.dart'; @docImport 'list_tile.dart';

# showSearch()

```dart
Future<T?> showSearch<T>({required BuildContext context, required SearchDelegate<T> delegate, String? query = '', bool useRootNavigator = false, bool maintainState = false})
```
...
The `maintainState` argument is used to determine if the route should remain in memory when it is inactive (see [ModalRoute.maintainState] for more details]. By default, `maintainState` is `false`.
...
This animation is commonly used to animate [AnimatedIcon]s of [IconButton]s returned by [buildLeading] or [buildActions]. It can also be used to animate [IconButton]s contained within the route below the search page.
