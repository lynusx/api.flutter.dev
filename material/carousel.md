...
When [infinite] is true, the carousel will create an infinite loop of items, allowing continuous scrolling in both directions.
...
On desktop and web running on desktop platforms, dragging to scroll with a mouse is disabled by default to align with natural behavior.

To further align expected behavior like this, mouse input can scroll horizontally by pressing the shift key while scrolling with the mouse wheel.

This key-driven behavior is dictated by the [ScrollBehavior.pointerAxisModifiers], while [ScrollBehavior.dragDevices] manages what devices can drag a scrollable.
...
```dart
CarouselView({dynamic key, EdgeInsets? padding, Color? backgroundColor, double? elevation, ShapeBorder? shape, Clip? itemClipBehavior, WidgetStateProperty<Color?>? overlayColor, bool itemSnapping = false, double shrinkExtent = 0.0, CarouselController? controller, Axis scrollDirection = Axis.horizontal, bool reverse = false, ValueChanged<int>? onTap, bool enableSplash = true, bool infinite = false, required double? itemExtent, required List<Widget> children, ValueChanged<int>? onIndexChanged})
```
...
```dart
CarouselView.weighted({dynamic key, EdgeInsets? padding, Color? backgroundColor, double? elevation, ShapeBorder? shape, Clip? itemClipBehavior, WidgetStateProperty<Color?>? overlayColor, bool itemSnapping = false, double shrinkExtent = 0.0, CarouselController? controller, Axis scrollDirection = Axis.horizontal, bool reverse = false, bool consumeMaxWeight = true, ValueChanged<int>? onTap, bool enableSplash = true, bool infinite = false, required List<int>? flexWeights, required List<Widget> children, ValueChanged<int>? onIndexChanged})
```
...
{@tool snippet} This example shows how to create a weighted carousel with lazy loading:
...
{@template flutter.material.CarouselView.onIndexChanged} A callback invoked when the leading item changes.

The leading item is the first visible item in the carousel view.

The callback fires only when the leading item is completely out of view, whether due to user interaction or programmatic scrolling. If the leading item remains partially visible, the leading index will not change and the callback will not be invoked. {@endtemplate}
...
```dart
CarouselScrollPhysics({dynamic parent})
```
...
For [CarouselView], this will scroll the carousel so the item at [index] becomes the leading item.

If the [index] is less than 0, the carousel will scroll to the first item. If the [index] is greater than the number of items, the carousel will scroll to the last item.

For [CarouselView.weighted], animates to make the item at [index] occupy the primary, most prominent position determined by the largest weight in `flexWeights`.
...
When [CarouselView.infinite] is true, the animation always scrolls forward to reach the target item, even if the item is closer in the backward direction.
