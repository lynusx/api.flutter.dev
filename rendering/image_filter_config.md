...
```dart
ImageFilterContext({required ui.Rect bounds})
```
...
- [ImageFilterContext], which provides the layout information used to resolve this configuration.
- [ImageFilterConfig.blur], which can be used to create a "bounded blur" that limits sampling to the object's boundaries.
...
```dart
ImageFilterConfig(ui.ImageFilter filter)
```
...
The [filter] property of an instance created with this constructor returns the original filter.

### ImageFilterConfig.blur()

```dart
ImageFilterConfig.blur({double sigmaX, double sigmaY, ui.TileMode tileMode, bool bounded})
```

Creates a configuration for a Gaussian blur.

The `sigmaX` and `sigmaY` arguments are the standard deviation of the Gaussian kernel in the x and y directions, respectively.
