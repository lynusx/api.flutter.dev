...
{@tool snippet} In this simple case, the text will be rendered at font size 10, however, the vertical height of each line will be the strut height (Roboto in font size 30 \* 1.5) as the text itself is shorter than the strut.
...
{@tool snippet} Here, strut is used to enable strange and overlapping text to achieve unique effects. The `M`s in lines 2 and 3 are able to extend above their lines and fill empty space in lines above. The [forceStrutHeight] is enabled and functions as a 'grid' for the glyphs to draw on.
...
{@tool snippet} This example uses forceStrutHeight to create a 'drop cap' for the 'T' in 'The'. By locking the line heights to the metrics of the 14pt serif font, we are able to lay out a large 37pt 'T' on the second line to take up space on both the first and second lines.
...
When a non-null [height] is specified, after accommodating the imaginary strut glyph, the remaining vertical space from the allotted `fontSize * height` logical pixels will be distributed over and under the strut, according to the [leadingDistribution] property.
...
The "missing" properties of the this strut style are _filled_ by the properties of the provided [TextStyle]. This is possible because [StrutStyle] shares many of the same basic properties as [TextStyle].
