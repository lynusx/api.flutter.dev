@docImport 'package:flutter/foundation.dart'; @docImport 'package:flutter/material.dart';

@docImport 'animated_size.dart'; @docImport 'transitions.dart';
...
- [ValueListenableBuilder], a widget whose content stays synced with a [ValueListenable] instead of a [Tween].

### TweenAnimationBuilder()

```dart
TweenAnimationBuilder({dynamic key, required Tween<T> tween, required Duration duration, dynamic curve, required ValueWidgetBuilder<T> builder, dynamic onEnd, Widget? child})
```
...
If the pre-built subtree is passed as the child parameter, the [TweenAnimationBuilder] will pass it back to the [builder] function so that it can be incorporated into the build.
