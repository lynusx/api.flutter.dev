...
{@tool dartpad} This example demonstrates how `InheritedTheme.capture()` can be used to wrap the contents of a new route with the inherited themes that are present when the route was built - but are not present when route is actually shown.
...
** See code in examples/api/lib/widgets/inherited_theme/inherited_theme.0.dart ** {@end-tool}
...
The `to` context must be an ancestor of `context`. If `to` is not specified, all inherited themes up to the root of the widget tree are captured.

After calling this method, the themes present between `context` and `to` are frozen for the provided `child`. If the themes (or their theme data) change in the original subtree, those changes will not be visible to the wrapped `child` - unless this method is called again to re-wrap the child.
...
Returns a [CapturedThemes] object that includes all the [InheritedTheme]s between the given `from` and `to` [BuildContext]s.
