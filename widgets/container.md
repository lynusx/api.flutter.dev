...
@docImport 'decorated_sliver.dart'; @docImport 'implicit_animations.dart'; @docImport 'transitions.dart';
...
{@tool snippet} This example shows a 48x48 amber square (placed inside a [Center] widget in case the parent widget has its own opinions regarding the size that the [Container] should take), with a margin so that it stays away from neighboring widgets:
